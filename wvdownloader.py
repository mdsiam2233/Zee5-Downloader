...
class WvDownloader(object):
    ...
    def get_track_download_thread(self, track, bar_position):
        ...
        def do_download():
            ...
            pbar.close()
            """
            aria2_p = subprocess.run(["binaries\\aria2c", "-x16", "-s16", "--summary-interval=0", "--enable-color=false", track.url, f"--dir={output_dir}", f"--out={output_name}"])
    ...
    def run(self):
        ...
        success, data = self.client.get_track_and_init_info()
        if not success:
        ...
        vt, ats, sts, init_data_b64, cert_data_b64 = data
        ...
        for track in enc_tracks:
            wvdecrypt_config = WvDecryptConfig(self.filename, track.get_type(), track.id, self.config.license, 
                                               init_data_b64, cert_data_b64=cert_data_b64)
